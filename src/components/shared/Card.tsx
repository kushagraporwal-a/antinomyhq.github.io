...
const Card = ({children, variant = "default"}: CardProps): JSX.Element => {
  const CardVariant = {
    ...
    warning:
      "custom-cards bg-[#DED7CA] dark:bg-[#2F2D2A] border-l-4 border-l-solid border-[#C68400] dark:border-[#F9A705] p-8 rounded-[12px] text-black dark:text-white mt-4 mb-[20px]",
    information:
      "custom-cards bg-[#D5E7DB] dark:bg-[#272D29] border-l-4 border-l-solid border-[#158B00] dark:border-[#22E000] p-8 rounded-[12px] text-black dark:text-white mt-4 mb-[20px]",
  ...
  return (
    <div className={`text-justify text-title-small font-normal h-[100%] ${CardVariant[variant]}`}>
      ...
      {variant === "information" && <Info className="text-[#158B00] dark:text-[#22E000] block mb-3" />}
      {variant === "tip" && <Lightbulb className="text-[#5261FF] dark:text-[#5968FF] block mb-3" />}
      {children}
    </div>
  )
}
