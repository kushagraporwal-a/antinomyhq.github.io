        ...
        <div ref={cardsRef} className="flex absolute gap-6 top-40 md:top-72 p-3 xl:top-[320px]">
          {cardsData.map((card, idx) => {
            return (
              <Card key={idx}>
                <div
                  className={`px-8 py-8 rounded-xl min-w-[380px] max-w-[380px] border-[11px] border-[#FFFFFF] dark:border-[#181D27] border-solid bg-transparent shadow-[0px_0px_4px_0px_#30EDE6] hover:cursor-pointer hover:bg-custom-radial hover:transition-colors hover:duration-500`}
                >
                  <div className="flex flex-col gap-3">
                    <img src={card.imageUrl} alt="Feedback" height={80} width={80} className="grayscale" />
                    <span className="text-[#454545] dark:text-[#A1A1A1] font-kanit text-title-small font-light">{card.title}</span>
                    <span className="text-[#454545] dark:text-white opacity-50 text-title-tiny font-normal">- {card.author}</span>
                  </div>
                </div>
              </Card>
            )
