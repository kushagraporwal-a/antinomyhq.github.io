...
import type {Props, PropVersions, PropVersionItem} from "@theme/NavbarItem/DocsVersionDropdownNavbarItem"
...
function getVersionItems(versions: GlobalVersion[], configs?: PropVersions): VersionItem[] {
  if (configs) {
    // Collect all the versions we have
    const versionMap = new Map<string, GlobalVersion>(versions.map((version) => [version.name, version]))

    const toVersionItem = (name: string, config?: PropVersionItem): VersionItem => {
...
export default function DocsVersionDropdownNavbarItem({
  ...
  function versionItemToLink({version, label}: VersionItem): LinkLikeNavbarItemProps {
  ...
  const dropdownTo =
    mobile && items.length > 1 ? undefined : getVersionTargetDoc(displayedVersionItem.version, activeDocContext).path
