...
You can easily switch between agents during a session using the `/fuse`, `/forge`, or `/agent` commands. The Muse Agent is especially useful for reviewing potential changes before they're implemented, while the Forge Agent streamlines the development process by handling implementation details for you.
