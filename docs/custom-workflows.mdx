...
A common pattern involves a powerful generalist agent for main tasks, with specialist agents for specific needs:

```yaml
agents:
  - id: code_reviewer # Specialist
    model: *efficiency_model
    tools:
      - tool_forge_fs_read
      - tool_forge_event_dispatch
...
For complex projects, create hierarchical teams with managers and workers:
...
- **Start Simple**: Begin with the default configuration before creating complex workflows
- **Specialize Agents**: Give each agent a clear, specific role
- **Use Appropriate Models**: Match model capabilities to agent responsibilities
- **Test Incrementally**: Add one agent at a time and test thoroughly
- **Document Your Workflows**: Include comments in your YAML to explain agent roles and interactions
