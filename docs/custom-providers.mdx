...
1. **`OPENROUTER_API_KEY`** - Second priority (multiple models)
2. **`OPENAI_API_KEY`** - Third priority (official OpenAI)
3. **`ANTHROPIC_API_KEY`** - Lowest priority (official Anthropic)
...
# Result: Forge uses project_openai_key (highest priority)
